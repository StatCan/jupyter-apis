describe('Main tables', () => {
  ...
  describe('Notebooks table', ()=>{
    ...
    it('should have the Notebooks table', () => {
      ...
      cy.get('[data-cy-table-id="notebooks-table"] > tbody > tr').should('have.length', 7);

      cy.get('[data-cy-toolbar-button="New Notebook"]').should('exist').and('be.enabled');
    });
    ...
    it('should start a notebook', ()=>{
      cy.get('[data-cy-table-id="notebooks-table"]').find(`[data-cy-resource-table-row="Name"]`).contains('a0-new-image').parent().parent().find('[data-cy-resource-table-action-button="connect"] > button').should('be.disabled');
      cy.get('[data-cy-table-id="notebooks-table"]').find(`[data-cy-resource-table-row="Name"]`).contains('a0-new-image').parent().parent().find('[data-cy-resource-table-action-icon="start-stop"] > button').should('have.text', ' play_arrow\n');
      cy.intercept('PATCH', '/api/namespaces/kubeflow-user/notebooks/a0-new-image', {"success":true,"status":200}).as('mockStartNotebook');
      cy.get('[data-cy-table-id="notebooks-table"]').find(`[data-cy-resource-table-row="Name"]`).contains('a0-new-image').parent().parent().find('[data-cy-resource-table-action-icon="start-stop"]').click();
      cy.wait('@mockStartNotebook').its('response.statusCode').should('eq', 200);
    });
    ...
    it('should have protected B icon on protected B notebook', ()=>{
      cy.get('[data-cy-table-id="notebooks-table"]').find(`[data-cy-resource-table-row="Name"]`).contains('test-pro-b').parent().parent().find('app-protb-icon').should('exist');
    });
    ...
    it('should filter the notebook table', ()=>{
      ...
      cy.get('[data-cy-table-filter-id="notebooks-table"]').type('pro-b{enter}');
      cy.get('[data-cy-table-id="notebooks-table"] > tbody > tr').should('have.length', 1);
      ...
      // generic filter
      cy.get('[data-cy-table-filter-id="notebooks-table"]').type('jupyter-scipy{enter}');
      cy.get('[data-cy-table-id="notebooks-table"] > tbody > tr').should('have.length', 4);
    ...
    it('should open the notebook details page', ()=>{
      cy.get('[data-cy-table-id="notebooks-table"]').find(`[data-cy-resource-table-row="Name"]`).contains('a-dog-breed-katib').click();
      cy.url().should('eq', 'http://localhost:4200/notebook/details/kubeflow-user/a-dog-breed-katib');
    });
  });
  ...
  describe('Volumes table', ()=>{
    ...
    it('should delete volume', ()=>{
      ...
      cy.get('[data-cy-table-id="volumes-table"]').find(`[data-cy-resource-table-row="Name"]`).contains('a-pvc-phase-waiting-viewer-uninitialized').parent().parent().find(`[data-cy-resource-table-row="Used by"]`).should('have.text', ' test-notebook-1\n');
      cy.get('[data-cy-table-id="volumes-table"]').find(`[data-cy-resource-table-row="Name"]`).contains('a-pvc-phase-waiting-viewer-uninitialized').parent().parent().find('app-delete-button').find('button').should('have.text', ' delete\n').and('be.disabled');
    ...
    it('should have protected B icon on protected B volume', ()=>{
      cy.get('[data-cy-table-id="volumes-table"]').find(`[data-cy-resource-table-row="Name"]`).contains('test-pro-b-volume').parent().parent().find('app-protb-icon').should('exist');
    });
    ...
    it('should filter the volume table', ()=>{
      ...
      cy.get('[data-cy-table-id="volumes-table"] > tbody > tr').should('have.length', 4);
    ...
    it('should open the volume details page', ()=>{
      cy.get('[data-cy-table-id="volumes-table"]').find(`[data-cy-resource-table-row="Name"]`).contains('a-pvc-phase-ready-viewer-ready').click();
      cy.url().should('eq', 'http://localhost:4200/volume/details/kubeflow-user/a-pvc-phase-ready-viewer-ready');
    });

    it('should open notebook details page from volume table used-by column', ()=>{
      cy.get('[data-cy-table-id="volumes-table"]').find(`[data-cy-resource-table-row="Name"]`).contains('a-pvc-phase-waiting-viewer-uninitialized').parent().parent().find(`[data-cy-resource-table-row="Used by"]`).should('have.text', ' test-notebook-1\n');
      cy.get('[data-cy-table-id="volumes-table"]').find(`[data-cy-resource-table-row="Name"]`).contains('a-pvc-phase-waiting-viewer-uninitialized').parent().parent().find(`[data-cy-resource-table-row="Used by"]`).click();
      cy.url().should('eq', 'http://localhost:4200/notebook/details/kubeflow-user/test-notebook-1');
    });
  });
  ...
  describe('Kubecost table', ()=>{
    ...
    it('should have the Kubecost table', ()=>{
      ...
      cy.get('[data-cy-table-id="kubecost-table"] > tbody > tr').should('have.length', 1);
      cy.get('[data-cy-table-id="kubecost-table"] > tbody > tr > [data-cy-resource-table-row="CPUs"]').should('have.text', ' $1.07 ');
      cy.get('[data-cy-table-id="kubecost-table"] > tbody > tr > [data-cy-resource-table-row="GPUs"]').should('have.text', ' $0.00 ');
      cy.get('[data-cy-table-id="kubecost-table"] > tbody > tr > [data-cy-resource-table-row="RAM"]').should('have.text', ' $0.28 ');
      cy.get('[data-cy-table-id="kubecost-table"] > tbody > tr > [data-cy-resource-table-row="Storage"]').should('have.text', ' $0.04 ');
      cy.get('[data-cy-table-id="kubecost-table"] > tbody > tr > [data-cy-resource-table-row="Total"]').should('have.text', ' $1.39 ');
    });
    ...
    it('should change cost table time window', ()=>{
      ...
      cy.intercept('GET', `/api/namespaces/kubeflow-user/cost/allocation?aggregation=namespace&namespace=kubeflow-user&window=week`, {
        "code":200,
        "data":{
      ...
      cy.wait('@mockKubecostRequest');
      cy.get('[data-cy-table-id="kubecost-table"] > tbody > tr').should('have.length', 1);
      cy.get('[data-cy-table-id="kubecost-table"] > tbody > tr > [data-cy-resource-table-row="CPUs"]').should('have.text', ' $6.42 ');
      cy.get('[data-cy-table-id="kubecost-table"] > tbody > tr > [data-cy-resource-table-row="GPUs"]').should('have.text', ' $0.50 ');
      cy.get('[data-cy-table-id="kubecost-table"] > tbody > tr > [data-cy-resource-table-row="RAM"]').should('have.text', ' $1.85 ');
      cy.get('[data-cy-table-id="kubecost-table"] > tbody > tr > [data-cy-resource-table-row="Storage"]').should('have.text', ' $0.20 ');
      cy.get('[data-cy-table-id="kubecost-table"] > tbody > tr > [data-cy-resource-table-row="Total"]').should('have.text', ' $8.97 ');
    });
  });
});
